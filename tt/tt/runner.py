...
def _translate_bare(
    ...
    """Translate a TS file with no stub — emit a plain Python module.

    Methods that translate successfully are placed inside a class (using the
    TS class name); methods that fail are emitted as ``pass`` stubs so the
    ...
    header += list(dict.fromkeys(extra_imports))
    header.append("")
    cname = class_name or "TranslatedModule"
    body: List[str] = [f"class {cname}:"]
    if not translated:
        body.append("    pass")
    else:
        for name, src in translated.items():
            body.append(src)
            body.append("")
...
def _translate_entry(
    ...
    if out_path is None:
        return FileReport(out_path=Path("<unknown>"), error="missing output path")
    ...
    try:
        if stub_path is None:
            return _translate_bare(
                ts_path, out_path, import_map, entry.get("class_name")
            )
